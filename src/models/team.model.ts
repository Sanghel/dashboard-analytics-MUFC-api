...
import { DbTeamStats } from '../types/db';

export const upsertTeamStats = async (stats: DbTeamStats): Promise<void> => {
  const sql = `
...
export const getTeamStats = async (teamId: number, season: number, leagueId: number): Promise<DbTeamStats | null> => {
  const [rows] = await pool.execute<RowDataPacket[]>(
    'SELECT * FROM team_stats WHERE team_id = ? AND season = ? AND league_id = ?',
    [teamId, season, leagueId]
  );
  return (rows[0] as DbTeamStats) || null;
};
